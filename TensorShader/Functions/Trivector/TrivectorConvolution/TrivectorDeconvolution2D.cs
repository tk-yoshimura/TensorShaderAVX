...
namespace TensorShader {
    public abstract partial class VariableNode {
        ...
        public static VariableNode TrivectorDeconvolution2D(VariableNode x, VariableNode w, int stride, bool gradmode = false, Shape outshape = null) {
            if (outshape == null) {
                int outwidth = (x.Shape.Width - 1) * stride + w.Shape.Width;
                int outheight = (x.Shape.Height - 1) * stride + w.Shape.Height;

                outshape = Shape.Map2D(w.Shape.InChannels / 4 * 3, outwidth, outheight, x.Shape.Batch);
            }

            Function function =
                new Functions.TrivectorConvolution.TrivectorDeconvolution2D(outshape, w.Shape, stride, gradmode);
    ...
    public partial class Tensor {
        /// <summary>3次元ベクトル2次元逆畳み込み</summary>
        public static Tensor TrivectorDeconvolution2D(Tensor x, Tensor w, int stride, bool gradmode = false, Shape outshape = null) {
            if (outshape == null) {
                int outwidth = (x.Shape.Width - 1) * stride + w.Shape.Width;
                int outheight = (x.Shape.Height - 1) * stride + w.Shape.Height;
...
namespace TensorShader.Functions.TrivectorConvolution {
    ...
    internal class TrivectorDeconvolution2D : Function {
        ...
        public TrivectorDeconvolution2D(Shape outshape, Shape kernelshape, int stride, bool gradmode) :
            ...
            this.InShape = Shape.Map2D(kernelshape.OutChannels * 3, inwidth, inheight, outshape.Batch);
