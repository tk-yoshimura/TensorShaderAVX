...
namespace TensorShader {
    public abstract partial class VariableNode {
        ...
        public static VariableNode PointwiseConvolution3D(VariableNode x, VariableNode w) {
            Function function =
                new Functions.Connection3D.PointwiseConvolution(x.Shape, w.Shape);
    ...
    public partial class Tensor {
        /// <summary>ポイントごとの3次元畳み込み</summary>
        public static Tensor PointwiseConvolution3D(Tensor x, Tensor w) {
            Functions.Connection3D.PointwiseConvolution function =
                new Functions.Connection3D.PointwiseConvolution(x.Shape, w.Shape);
...
namespace TensorShader.Functions.Connection3D {
    ...
    internal class PointwiseConvolution : Function {
        ...
        public PointwiseConvolution(Shape inshape, Shape kernelshape) :
            ...
            this.OutShape = Shape.Map3D(kernelshape.OutChannels, inshape.Width, inshape.Height, inshape.Depth, inshape.Batch);
        ...
        internal override (Tensor[] tensors, Operator ope) GenerateOperator(Tensor[] intensors, Tensor[] outtensors) {
            ...
            return (new Tensor[] { intensors[0], intensors[1], outtensors[0] },
                    new Operators.Connection3D.PointwiseConvolution(
                        InShape.Width, InShape.Height, InShape.Depth,
                        InShape.Channels, OutShape.Channels, InShape.Batch));
        }
    }
}
