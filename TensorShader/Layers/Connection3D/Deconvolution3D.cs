...
namespace TensorShader.Layers {
    ...
    public class Deconvolution3D : Layer {
        /// <summary>重み</summary>
        ...
        public Deconvolution3D(int inchannels, int outchannels, int kwidth, int kheight, int kdepth, int stride, bool use_bias, PaddingMode pad_mode, string label)
            : base(label) {
            this.W = new ParameterField(
                new Tensor(Shape.Kernel3D(outchannels, inchannels, kwidth, kheight, kdepth)),
                Label + "/w",
        ...
        public Field Forward(Field x) {
            Field y = Deconvolution3D(x, W, Stride);
