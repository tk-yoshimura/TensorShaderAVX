    ...
    internal class SpaceToChannel : Operator {
        ...
        public SpaceToChannel(int inwidth, int inheight, int inchannels, int scale, int batch = 1) {
            ...
            this.arguments = new List<(ArgumentType type, Shape shape)>{
                ...
                (ArgumentType.Out, Shape.Map2D(checked(inchannels * scale * scale), checked(inwidth / scale), checked(inheight / scale), batch)),
            };
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Transform.SpaceToChannel2D((uint)InChannels, (uint)outmap.Width, (uint)outmap.Height, (uint)Batch, (uint)th, (uint)Scale, inmap.Buffer, outmap.Buffer);
            });
        }
