...
namespace TensorShader.Operators.Connection2D {
    ...
    internal class MaxPooling : Operator {
        ...
        public MaxPooling(int inwidth, int inheight, int channels, int stride, int batch = 1) {
            if (stride < 2) {
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Pool.MaxPool2D((uint)Channels, (uint)inmap.Width, (uint)inmap.Height, (uint)Batch, (uint)th, (uint)Stride, inmap.Buffer, outmap.Buffer);
            });
        }
