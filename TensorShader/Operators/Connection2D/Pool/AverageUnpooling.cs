...
namespace TensorShader.Operators.Connection2D {
    ...
    internal class AverageUnpooling : Operator {
        ...
        public AverageUnpooling(int outwidth, int outheight, int channels, int stride, int batch = 1) {
            if (stride < 2) {
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Pool.AverageUnpool2D((uint)Channels, (uint)outmap.Width, (uint)outmap.Height, (uint)Batch, (uint)th, (uint)Stride, inmap.Buffer, outmap.Buffer);
            });
        }
