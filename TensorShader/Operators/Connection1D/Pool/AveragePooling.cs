...
namespace TensorShader.Operators.Connection1D {
    ...
    internal class AveragePooling : Operator {
        ...
        public AveragePooling(int inwidth, int channels, int stride, int batch = 1) {
            if (stride < 2) {
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Pool.AveragePool1D((uint)Channels, (uint)inmap.Width, (uint)Batch, (uint)th, (uint)Stride, inmap.Buffer, outmap.Buffer);
            });
        }
