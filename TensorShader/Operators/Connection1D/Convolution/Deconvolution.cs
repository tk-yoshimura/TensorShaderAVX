...
namespace TensorShader.Operators.Connection1D {
    ...
    internal class Deconvolution : Operator {
        ...
        public Deconvolution(int inwidth, int inchannels, int outchannels, int kwidth, int batch = 1) {

            int outwidth = inwidth + kwidth - 1;

            this.arguments = new List<(ArgumentType type, Shape shape)>{
                (ArgumentType.In, Shape.Map1D(inchannels, inwidth, batch)),
                (ArgumentType.In, Shape.Kernel1D(outchannels, inchannels, kwidth)),
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            TensorShaderAvxBackend.Convolution.Deconvolution1D((uint)InChannels, (uint)OutChannels,
