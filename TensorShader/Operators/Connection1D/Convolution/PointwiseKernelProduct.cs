...
namespace TensorShader.Operators.Connection1D {
    ...
    internal class PointwiseKernelProduct : Operator {
        ...
        public PointwiseKernelProduct(int width, int inchannels, int outchannels, int batch = 1) {
            this.arguments = new List<(ArgumentType type, Shape shape)>{
                (ArgumentType.In, Shape.Map1D(inchannels, width, batch)),
                (ArgumentType.In, Shape.Map1D(outchannels, width, batch)),
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            TensorShaderAvxBackend.Convolution.PointwiseKernelProduct((uint)InChannels, (uint)OutChannels,
                                                                      (uint)(inmap1.Width * Batch),
