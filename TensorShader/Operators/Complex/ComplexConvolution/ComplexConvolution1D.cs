...
namespace TensorShader.Operators.ComplexConvolution {
    ...
    internal class ComplexConvolution1D : Operator {
        ...
        public ComplexConvolution1D(int inwidth, int inchannels, int outchannels, int kwidth, int stride, bool gradmode = false, int batch = 1) {
            if (stride < 1) {
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Complex.Convolution1D((uint)InChannels, (uint)OutChannels, 
                                                             (uint)inmap.Width, 
                                                             (uint)Batch, (uint)th, 
                                                             (uint)KernelWidth, 
