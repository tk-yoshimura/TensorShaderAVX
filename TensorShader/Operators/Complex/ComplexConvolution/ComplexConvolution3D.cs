...
namespace TensorShader.Operators.ComplexConvolution {
    ...
    internal class ComplexConvolution3D : Operator {
        ...
        public ComplexConvolution3D(int inwidth, int inheight, int indepth, int inchannels, int outchannels, int kwidth, int kheight, int kdepth, int stride, bool gradmode = false, int batch = 1) {
            if (stride < 1) {
            ...
            this.arguments = new List<(ArgumentType type, Shape shape)>{
                ...
                (ArgumentType.In, Shape.Kernel3D(inchannels, outchannels / 2, kwidth, kheight, kdepth)),
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Complex.Convolution3D((uint)InChannels, (uint)OutChannels, 
