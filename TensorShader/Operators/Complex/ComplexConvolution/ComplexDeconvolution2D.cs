...
namespace TensorShader.Operators.ComplexConvolution {
    ...
    internal class ComplexDeconvolution2D : Operator {
        ...
        public ComplexDeconvolution2D(int outwidth, int outheight, int inchannels, int outchannels, int kwidth, int kheight, int stride, bool gradmode = false, int batch = 1) {
            if (stride < 1) {
            ...
            this.arguments = new List<(ArgumentType type, Shape shape)>{
                ...
                (ArgumentType.In, Shape.Kernel2D(outchannels, inchannels / 2, kwidth, kheight)),
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Complex.Deconvolution2D((uint)InChannels, (uint)OutChannels, 
