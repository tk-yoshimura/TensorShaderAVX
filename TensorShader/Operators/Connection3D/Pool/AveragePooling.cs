...
namespace TensorShader.Operators.Connection3D {
    ...
    internal class AveragePooling : Operator {
        ...
        public AveragePooling(int inwidth, int inheight, int indepth, int channels, int stride, int batch = 1) {
            if (stride < 2) {
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Pool.AveragePool3D((uint)Channels, (uint)inmap.Width, (uint)inmap.Height, (uint)inmap.Depth, (uint)Batch, (uint)th, (uint)Stride, inmap.Buffer, outmap.Buffer);
            });
        }
