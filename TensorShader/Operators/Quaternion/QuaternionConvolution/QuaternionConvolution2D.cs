...
namespace TensorShader.Operators.QuaternionConvolution {
    ...
    internal class QuaternionConvolution2D : Operator {
        ...
        public QuaternionConvolution2D(int inwidth, int inheight, int inchannels, int outchannels, int kwidth, int kheight, int stride, bool gradmode = false, int batch = 1) {
            if (stride < 1) {
            ...
            this.arguments = new List<(ArgumentType type, Shape shape)>{
                ...
                (ArgumentType.In, Shape.Kernel2D(inchannels, outchannels / 4, kwidth, kheight)),
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Quaternion.Convolution2D((uint)InChannels, (uint)OutChannels, 
