...
namespace TensorShader.Operators.QuaternionConvolution {
    ...
    internal class QuaternionDeconvolution1D : Operator {
        ...
        public QuaternionDeconvolution1D(int outwidth, int inchannels, int outchannels, int kwidth, int stride, bool gradmode = false, int batch = 1) {
            if (stride < 1) {
            ...
            this.arguments = new List<(ArgumentType type, Shape shape)>{
                ...
                (ArgumentType.In, Shape.Kernel1D(outchannels, inchannels / 4, kwidth)),
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Quaternion.Deconvolution1D((uint)InChannels, (uint)OutChannels, 
                                                                  (uint)outmap.Width,
                                                                  (uint)Batch, (uint)th,
                                                                  (uint)KernelWidth, 
