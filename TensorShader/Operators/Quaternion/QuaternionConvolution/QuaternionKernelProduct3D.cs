...
namespace TensorShader.Operators.QuaternionConvolution {
    ...
    internal class QuaternionKernelProduct3D : Operator{
        ...
        public QuaternionKernelProduct3D(int inwidth, int inheight, int indepth, int inchannels, int outchannels, int kwidth, int kheight, int kdepth, int stride, bool transpose = false, int batch = 1) {
            if (stride < 1) {
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, OutChannels / 4, (outch) => {
                TensorShaderAvxBackend.Quaternion.KernelProduct3D((uint)InChannels, (uint)OutChannels, 
