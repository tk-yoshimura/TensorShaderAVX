...
namespace TensorShader.Operators.QuaternionConvolution {
    ...
    internal class QuaternionDeconvolution3D : Operator {
        ...
        public QuaternionDeconvolution3D(int outwidth, int outheight, int outdepth, int inchannels, int outchannels, int kwidth, int kheight, int kdepth, int stride, bool gradmode = false, int batch = 1) {
            if (stride < 1) {
            ...
            this.arguments = new List<(ArgumentType type, Shape shape)>{
                ...
                (ArgumentType.In, Shape.Kernel3D(outchannels, inchannels / 4, kwidth, kheight, kdepth)),
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Quaternion.Deconvolution3D((uint)InChannels, (uint)OutChannels, 
