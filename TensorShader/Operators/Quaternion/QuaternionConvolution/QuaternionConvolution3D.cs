...
namespace TensorShader.Operators.QuaternionConvolution {
    ...
    internal class QuaternionConvolution3D : Operator {
        ...
        public QuaternionConvolution3D(int inwidth, int inheight, int indepth, int inchannels, int outchannels, int kwidth, int kheight, int kdepth, int stride, bool gradmode = false, int batch = 1) {
            if (stride < 1) {
            ...
            this.arguments = new List<(ArgumentType type, Shape shape)>{
                ...
                (ArgumentType.In, Shape.Kernel3D(inchannels, outchannels / 4, kwidth, kheight, kdepth)),
        ...
        public override void Execute(params Tensor[] tensors) {
            ...
            Parallel.For(0, Batch, (th) => {
                TensorShaderAvxBackend.Quaternion.Convolution3D((uint)InChannels, (uint)OutChannels, 
