...
namespace TensorShader {
    public partial class Field {
        ...
        public static Field ChannelToSpace2D(Field x, int scale) {
            Field y = new Field();
            Link link = new Links.Connection2D.ChannelToSpace(x, y, scale);
...
namespace TensorShader.Links.Connection2D {
    ...
    public class ChannelToSpace : Link {
        ...
        public override void Forward() {
            Y.AssignValue(ChannelToSpace2D(X.Value, Scale));
        }
        ...
        public override void Backward() {
            ...
            if (X.EnableBackprop) {
                X.AddGrad(SpaceToChannel2D(Y.Grad, Scale));
            }
        }
    }
}
