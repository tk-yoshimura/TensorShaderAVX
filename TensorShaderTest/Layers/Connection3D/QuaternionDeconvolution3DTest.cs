...
namespace TensorShaderTest.Layers {
    ...
    public class QuaternionDeconvolution3DTest {
        ...
        public void ExecuteTest() {
            int inchannels = 8, outchannels = 12, inwidth = 13, inheight = 17, indepth = 19, kwidth = 3, kheight = 5, kdepth = 7, stride = 2, batch = 7;

            VariableField x = new Tensor(Shape.Map3D(inchannels, inwidth, inheight, indepth, batch));

            Layer layer = new QuaternionDeconvolution3D(inchannels, outchannels, kwidth, kheight, kdepth, stride, use_bias:true, pad_mode:PaddingMode.Edge, "conv");
