...
namespace TensorShaderTest.Layers {
    ...
    public class QuaternionDeconvolution2DTest {
        ...
        public void ExecuteTest() {
            int inchannels = 8, outchannels = 12, inwidth = 13, inheight = 17, kwidth = 3, kheight = 5, stride = 2, batch = 7;

            VariableField x = new Tensor(Shape.Map2D(inchannels, inwidth, inheight, batch));

            Layer layer = new QuaternionDeconvolution2D(inchannels, outchannels, kwidth, kheight, stride, use_bias:true, pad_mode:PaddingMode.Edge, "conv");
