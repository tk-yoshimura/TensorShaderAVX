...
void TensorShaderAvxBackend::Channelwise::DivLVector(unsigned int vector_length, unsigned int map_length, cli::array<float>^ srcvector, cli::array<float>^ srcmap, cli::array<float>^ dstmap) {
    ...
    if (vector_length == 1) {
        Elementwise::DivConstant(0, map_length, srcvector[0], srcmap, dstmap);
        return;
    }
