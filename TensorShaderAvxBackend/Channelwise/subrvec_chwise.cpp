...
void subrvec_chwise(unsigned int veclength, unsigned int maplength, float* srcvec_ptr, float* srcmap_ptr, float* dstmap_ptr) {
...
void TensorShaderAvxBackend::Channelwise::SubRVector(unsigned int vector_length, unsigned int map_length, AvxArray<float>^ srcvector, AvxArray<float>^ srcmap, AvxArray<float>^ dstmap) {
    ...
    if (vector_length == 1) {
        Elementwise::SubConstant(map_length, -srcvec_ptr[0], srcmap, dstmap);
        return;
    }

    subrvec_chwise(vector_length, map_length, srcvec_ptr, srcmap_ptr, dstmap_ptr);
}
