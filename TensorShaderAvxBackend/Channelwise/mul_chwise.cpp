...
void mul_chwise(unsigned int veclength, unsigned int maplength, float* srcvec_ptr, float* srcmap_ptr, float* dstmap_ptr) {
...
void TensorShaderAvxBackend::Channelwise::Mul(unsigned int vector_length, unsigned int map_length, cli::array<float>^ srcvector, cli::array<float>^ srcmap, cli::array<float>^ dstmap) {
    ...
    mul_chwise(vector_length, map_length, srcvec_ptr, srcmap_ptr, dstmap_ptr);
}
