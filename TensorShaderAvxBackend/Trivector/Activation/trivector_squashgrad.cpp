...
void TensorShaderAvxBackend::Trivector::SquashGrad(unsigned int index, unsigned int length, cli::array<float>^ src1, cli::array<float>^ src2, cli::array<float>^ dst) {
    ...
    trivector_squashgrad(length, src1_ptr + index, src2_ptr + index, dst_ptr + index);
}
