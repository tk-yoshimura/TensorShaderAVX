...
void trivector_squash(unsigned int length, float* src_ptr, float* dst_ptr) {
...
void TensorShaderAvxBackend::Trivector::Squash(unsigned int index, unsigned int length, cli::array<float>^ src, cli::array<float>^ dst) {
    ...
    trivector_squash(length, src_ptr + index, dst_ptr + index);
}