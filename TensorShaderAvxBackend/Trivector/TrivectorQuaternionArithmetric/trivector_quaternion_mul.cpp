...
void TensorShaderAvxBackend::Trivector::Mul(unsigned int vectorindex, unsigned int quaternionindex, unsigned int vectorlength, cli::array<float>^ src_vector, cli::array<float>^ src_quaternion, cli::array<float>^ dst_vector) {
    ...
    trivector_mul(vectorlength, v_ptr + vectorindex, q_ptr + quaternionindex, u_ptr + vectorindex);
}