...
void trivector_dense(unsigned int inchannels, unsigned int outchannels, unsigned int th, float* inmap_ptr, float* outmap_ptr, float* kernel_ptr) {
    ...
    for (unsigned int outch = 0, koutch = 0; outch < outchannels; outch += 3, koutch++) {
        ...
        for (unsigned int inch = 0, kinch = 0; inch < inchannels; inch += 3, kinch += 4) {
            ...
            __m256d q = _mm256_cvtps_pd(_mm_loadu_ps(kernel_ptr + kinch + kernelinchannels * koutch));
...
void TensorShaderAvxBackend::Trivector::Dense(unsigned int inchannels, unsigned int outchannels, unsigned int batch, unsigned int th, bool gradmode,
    ...
    if (gradmode) {
        trivector_dense_grad(inchannels, outchannels, th, inmap_ptr, outmap_ptr, kernel_ptr);
    }
    else {
        trivector_dense(inchannels, outchannels, th, inmap_ptr, outmap_ptr, kernel_ptr);
    }
}
