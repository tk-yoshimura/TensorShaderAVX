...
void trivector_transposedense(unsigned int inchannels, unsigned int outchannels, unsigned int th, float* inmap_ptr, float* outmap_ptr, float* kernel_ptr) {
...
void trivector_transposedense_grad(unsigned int inchannels, unsigned int outchannels, unsigned int th, float* inmap_ptr, float* outmap_ptr, float* kernel_ptr) {
    const unsigned int inmap_offset = inchannels * th, outmap_offset = outchannels * th;
    const unsigned int kerneloutchannels = outchannels / 3 * 4;
    const __m128i mask3 = TensorShaderAvxBackend::masktable_m128(3);
...
void TensorShaderAvxBackend::Trivector::TransposeDense(unsigned int inchannels, unsigned int outchannels, unsigned int batch, unsigned int th, bool gradmode,
    ...
    if (inchannels * outchannels * 4 / 9 > (unsigned int)kernel->Length) {
        throw gcnew System::ArgumentException();
    }
    if (th >= batch) {
        throw gcnew System::ArgumentException();
