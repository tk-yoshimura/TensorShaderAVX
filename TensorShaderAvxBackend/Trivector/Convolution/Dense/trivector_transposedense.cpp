...
void trivector_transposedense(unsigned int inchannels, unsigned int outchannels, unsigned int th, const float* __restrict inmap_ptr, float* __restrict outmap_ptr, const float* __restrict kernel_ptr) {
    const unsigned int inmap_offset = inchannels * th, outmap_offset = outchannels * th;
    const unsigned int kerneloutchannels = outchannels / 3 * 4;
    ...
    for (unsigned int outch = 0, koutch = 0; outch < outchannels; outch += 3, koutch += 4) {
        ...
        for (unsigned int inch = 0, kinch = 0; inch < inchannels; inch += 3, kinch++) {
            __m256d v = _mm256_cvtps_pd(_mm_maskload_ps(inmap_ptr + inch, mask3));
            __m256d q = _mm256_cvtps_pd(_mm_loadu_ps(kernel_ptr + koutch + kerneloutchannels * kinch));

            vq = _mm256_add_pd(_mm256_trivectormul_pd(v, q), vq);
        }

        _mm_maskstore_ps(outmap_ptr + outch, mask3, _mm256_cvtpd_ps(vq));
    }
}

void trivector_transposedense_grad(unsigned int inchannels, unsigned int outchannels, unsigned int th, const float* __restrict inmap_ptr, float* __restrict outmap_ptr, const float* __restrict kernel_ptr) {
    const unsigned int inmap_offset = inchannels * th, outmap_offset = outchannels * th;
    const unsigned int kerneloutchannels = outchannels / 3 * 4;
    const __m128i mask3 = TensorShaderAvxBackend::masktable_m128(3);
...
void TensorShaderAvxBackend::Trivector::TransposeDense(unsigned int inchannels, unsigned int outchannels, unsigned int batch, unsigned int th, bool gradmode,
    ...
    Util::CheckLength(inchannels * outchannels * 4 / 9, kernel);
    ...
    if (gradmode) {
        trivector_transposedense_grad(inchannels, outchannels, th, inmap_ptr, outmap_ptr, kernel_ptr);
    }
    else {
        trivector_transposedense(inchannels, outchannels, th, inmap_ptr, outmap_ptr, kernel_ptr);
    }
}
