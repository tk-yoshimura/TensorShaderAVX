            ...
            for (unsigned int outch = 0, koutch = 0; outch < outchannels; outch += 3, koutch += 4) {
                for (unsigned int ky = 0, oy = iy * stride; ky < kheight; ky++, oy++) {
                    for (unsigned int kx = 0, ox = ix * stride; kx < kwidth; kx++, ox++) {
                        __m256d vq = _mm256_cvtps_pd(_mm_maskload_ps(outmap_ptr + outch + outchannels * (ox + outwidth * oy), mask3));
                        ...
                        for (unsigned int inch = 0, kinch = 0; inch < inchannels; inch += 3, kinch++) {
                            ...
                            __m256d q = _mm256_cvtps_pd(_mm_load_ps(kernel_ptr + koutch + kerneloutchannels * (kinch + kernelinchannels * (kx + kwidth * ky))));
...
void TensorShaderAvxBackend::Trivector::Deconvolution2D(unsigned int inchannels, unsigned int outchannels, unsigned int outwidth, unsigned int outheight,
    ...
    outmap->Zeroset(outchannels * outwidth * outheight * batch);
    ...
    if (gradmode) {
        trivector_deconvolution_2d_grad(inchannels, outchannels,
                                        inwidth, outwidth, kwidth,
                                        inheight, outheight, kheight,
                                        0, 0, inmap_ptr, outmap_ptr, kernel_ptr);
    }
    else {
        trivector_deconvolution_2d(inchannels, outchannels,
                                   inwidth, outwidth, kwidth,
                                   inheight, outheight, kheight,
                                   0, 0, inmap_ptr, outmap_ptr, kernel_ptr);
    }
