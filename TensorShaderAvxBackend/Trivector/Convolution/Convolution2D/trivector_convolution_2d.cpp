...
void trivector_convolution_2d(unsigned int inchannels, unsigned int outchannels, 
    ...
    for (unsigned int oy = 0; oy < outheight; oy++) {
        for (unsigned int ox = 0; ox < outwidth; ox++) {
            for (unsigned int outch = 0, koutch = 0; outch < outchannels; outch += 3, koutch++) {
                ...
                for (unsigned int ky = 0, iy = oy * stride; ky < kheight; ky++, iy++) {
                    for (unsigned int kx = 0, ix = ox * stride; kx < kwidth; kx++, ix++) {
                        for (unsigned int inch = 0, kinch = 0; inch < inchannels; inch += 3, kinch += 4) {
                            __m256d v = _mm256_cvtps_pd(_mm_maskload_ps(inmap_ptr + inch + inchannels * (ix + inwidth * iy), mask3));
                            __m256d q = _mm256_cvtps_pd(_mm_loadu_ps(kernel_ptr + kinch + kernelinchannels * (koutch + kerneloutchannels * (kx + kwidth * ky))));
                ...
                _mm_maskstore_ps(outmap_ptr + outch + outchannels * (ox + outwidth * oy), mask3, _mm256_cvtpd_ps(vq));
            }
        }
    }
}

void trivector_convolution_2d_grad(unsigned int inchannels, unsigned int outchannels,
                                   unsigned int inwidth, unsigned int outwidth, unsigned int kwidth,
                                   unsigned int inheight, unsigned int outheight, unsigned int kheight,
                                   unsigned int stride, unsigned int th, float* inmap_ptr, float* outmap_ptr, float* kernel_ptr) {
...
void TensorShaderAvxBackend::Trivector::Convolution2D(unsigned int inchannels, unsigned int outchannels, unsigned int inwidth, unsigned int inheight,
    ...
    if (inchannels * outchannels * kwidth * kheight * 4 / 9 > (unsigned int)kernel->Length) {
        throw gcnew System::ArgumentException();
    }
    if (th >= batch) {
        throw gcnew System::ArgumentException();
    ...
    if (gradmode) {
        trivector_convolution_2d_grad(inchannels, outchannels, 
                                      inwidth, outwidth, kwidth,
                                      inheight, outheight, kheight,
                                      stride, th, inmap_ptr, outmap_ptr, kernel_ptr);
    }
    else {
        trivector_convolution_2d(inchannels, outchannels, 
                                 inwidth, outwidth, kwidth,
                                 inheight, outheight, kheight,
                                 stride, th, inmap_ptr, outmap_ptr, kernel_ptr);
    }
