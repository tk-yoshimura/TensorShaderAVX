...
void trivector_kernelproduct_2d(unsigned int inchannels, unsigned int outchannels, 
                ...
                for (unsigned int th = 0; th < batch; th++) {
                    for (unsigned int oy = 0, iy = ky; oy < outheight; oy++, iy += stride) {
                        for (unsigned int ox = 0, ix = kx; ox < outwidth; ox++, ix += stride) {
                            __m256d u = _mm256_cvtps_pd(_mm_maskload_ps(inmap_ptr + inch + inchannels * (ix + inwidth * (iy + inheight * th)), mask3));
                            __m256d v = _mm256_cvtps_pd(_mm_maskload_ps(outmap_ptr + outch + outchannels * (ox + outwidth * (oy + outheight * th)), mask3));
...
void trivector_kernelproduct_2d_transpose(unsigned int inchannels, unsigned int outchannels,
                                          unsigned int inwidth, unsigned int outwidth, unsigned int kwidth,
                                          unsigned int inheight, unsigned int outheight, unsigned int kheight,
                                          unsigned int stride, unsigned int batch, unsigned int outch,
                                          const float* __restrict inmap_ptr, float* __restrict outmap_ptr, float* kernelvalue_ptr, float* kernelgrad_ptr) {
