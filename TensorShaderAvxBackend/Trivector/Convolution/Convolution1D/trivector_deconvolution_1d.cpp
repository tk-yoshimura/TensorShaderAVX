...
void trivector_deconvolution_1d(unsigned int inchannels, unsigned int outchannels,
    ...
    for (unsigned int ix = 0; ix < inwidth; ix++) {
        for (unsigned int outch = 0, koutch = 0; outch < outchannels; outch += 3, koutch += 4) {
            for (unsigned int kx = 0, ox = ix * stride; kx < kwidth; kx++, ox++) {
                __m256d vq = _mm256_cvtps_pd(_mm_maskload_ps(outmap_ptr + outch + outchannels * ox, mask3));
                ...
                for (unsigned int inch = 0, kinch = 0; inch < inchannels; inch += 3, kinch++) {
                    ...
                    __m256d q = _mm256_cvtps_pd(_mm_loadu_ps(kernel_ptr + koutch + kerneloutchannels * (kinch + kernelinchannels * kx)));
...
void trivector_deconvolution_1d_grad(unsigned int inchannels, unsigned int outchannels,
                                     unsigned int inwidth, unsigned int outwidth, unsigned int kwidth,
                                     unsigned int stride, unsigned int th, float* inmap_ptr, float* outmap_ptr, float* kernel_ptr) {
...
void TensorShaderAvxBackend::Trivector::Deconvolution1D(unsigned int inchannels, unsigned int outchannels, unsigned int outwidth,
    ...
    if (gradmode) {
        trivector_deconvolution_1d_grad(inchannels, outchannels,
                                        inwidth, outwidth, kwidth,
                                        stride, th, inmap_ptr, outmap_ptr, kernel_ptr);
    }
    else {
        trivector_deconvolution_1d(inchannels, outchannels,
                                   inwidth, outwidth, kwidth,
                                   stride, th, inmap_ptr, outmap_ptr, kernel_ptr);
    }
}
