...
void TensorShaderAvxBackend::Aggregation::Max(unsigned int src_index, unsigned int src_length, cli::array<float>^ src, unsigned int dst_index, unsigned int dst_length, cli::array<float>^ dst, unsigned int slides) {
    ...
    if (dst_length <= 1) {
        max_1(src_length, src_ptr + src_index, dst_length, dst_ptr + dst_index, slides);
    }
    else if (dst_length <= 2) {
        max_2(src_length, src_ptr + src_index, dst_length, dst_ptr + dst_index, slides);
    }
    else if (dst_length <= 3) {
        max_3(src_length, src_ptr + src_index, dst_length, dst_ptr + dst_index, slides);
    }
    else if (dst_length <= 4) {
        max_4(src_length, src_ptr + src_index, dst_length, dst_ptr + dst_index, slides);
    }
    else if (dst_length <= 7) {
        max_5_7(src_length, src_ptr + src_index, dst_length, dst_ptr + dst_index, slides);
    }
    else if (dst_length <= 8) {
        max_8(src_length, src_ptr + src_index, dst_length, dst_ptr + dst_index, slides);
    }
    else {
        max_9_(src_length, src_ptr + src_index, dst_length, dst_ptr + dst_index, slides);
    }
}