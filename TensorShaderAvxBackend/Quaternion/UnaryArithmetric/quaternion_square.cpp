...
void quaternion_square(unsigned int length, const float* __restrict src_ptr, float* __restrict dst_ptr) {
...
void TensorShaderAvxBackend::Quaternion::Square(unsigned int length, AvxArray<float>^ src, AvxArray<float>^ dst) {
    ...
    quaternion_square(length, src_ptr, dst_ptr);
}