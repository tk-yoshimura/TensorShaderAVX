        ...
        for (unsigned int kx = 0; kx < kwidth; kx++) {
            ...
            for (unsigned int th = 0; th < batch; th++) {
                for (unsigned int ox = 0, ix = kx; ox < outwidth; ox++, ix += stride) {
                    __m256d u = _mm256_cvtps_pd(_mm_loadu_ps(inmap_ptr + inch + inchannels * (ix + inwidth * th)));
                    __m256d v = _mm256_cvtps_pd(_mm_loadu_ps(outmap_ptr + outch + outchannels * (ox + outwidth * th)));

                    uv = _mm256_add_pd(_mm256_quaternionmulkernelgrad_pd(u, v), uv);
                }
            }

            _mm_storeu_ps(kernel_ptr + inch + inchannels * (koutch + kerneloutchannels * kx), _mm256_cvtpd_ps(uv));
        }
    }
}
...
void quaternion_kernelproduct_1d_transpose(unsigned int inchannels, unsigned int outchannels, 
    ...
    for (unsigned int inch = 0; inch < inchannels; inch += 4) {
        for (unsigned int kx = 0; kx < kwidth; kx++) {
            ...
            _mm_storeu_ps(kernel_ptr + inch + inchannels * (koutch + kerneloutchannels * kx), _mm256_cvtpd_ps(vu));
        }
    }
}
