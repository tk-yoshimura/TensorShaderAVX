...
void quaternion_dense(unsigned int inchannels, unsigned int outchannels, unsigned int th, float* inmap_ptr, float* outmap_ptr, float* kernel_ptr) {
    ...
    for (unsigned int outch = 0, koutch = 0; outch < outchannels; outch += 4, koutch++) {
        ...
        for (unsigned int inch = 0; inch < inchannels; inch += 4) {
            ...
            __m256d v = _mm256_cvtps_pd(_mm_loadu_ps(kernel_ptr + inch + inchannels * koutch));
...
void quaternion_dense_grad(unsigned int inchannels, unsigned int outchannels, unsigned int th, float* inmap_ptr, float* outmap_ptr, float* kernel_ptr) {
    const unsigned int inmap_offset = inchannels * th, outmap_offset = outchannels * th;
