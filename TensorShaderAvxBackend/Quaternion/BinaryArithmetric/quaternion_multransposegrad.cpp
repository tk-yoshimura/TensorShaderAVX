...
__forceinline __m256 _mm256_quaternionmultransposegrad_ps(__m256 u, __m256 v) {
    ...
    __m256 v_xyzw = v;
    __m256 v_yxwz = _mm256_xor_ps(sign_pmpm, _mm256_permute_ps(v_xyzw, _MM_PERM_CDAB));
    __m256 v_zwxy = _mm256_xor_ps(sign_pmmp, _mm256_permute_ps(v_xyzw, _MM_PERM_BADC));
    __m256 v_wzyx = _mm256_xor_ps(sign_ppmm, _mm256_permute_ps(v_xyzw, _MM_PERM_ABCD));
...
void quaternion_multransposegrad(unsigned int length, float* src1_ptr, float* src2_ptr, float* dst_ptr) {
    ...
    for (unsigned int i = 0; i < j; i += 8) {
        ...
        __m256 y = _mm256_quaternionmultransposegrad_ps(x1, x2);
...
void TensorShaderAvxBackend::Quaternion::MulTransposeGrad(unsigned int index, unsigned int length, cli::array<float>^ src1, cli::array<float>^ src2, cli::array<float>^ dst) {
    ...
    quaternion_multransposegrad(length, src1_ptr + index, src2_ptr + index, dst_ptr + index);
}