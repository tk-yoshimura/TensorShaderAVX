...
void TensorShaderAvxBackend::Quaternion::Squash(unsigned int index, unsigned int length, cli::array<float>^ src, cli::array<float>^ dst) {
    ...
    quaternion_squash(length, src_ptr + index, dst_ptr + index);
}