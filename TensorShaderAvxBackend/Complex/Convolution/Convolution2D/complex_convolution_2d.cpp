...
void complex_convolution_2d(unsigned int inchannels, unsigned int outchannels, 
        ...
        for (unsigned int oy = 0; oy < outheight; oy++) {
            for (unsigned int ox = 0; ox < outwidth; ox++) {
                for (unsigned int outch = 0, koutch = 0; outch < outchannels; outch += 2, koutch++) {
                    ...
                    for (unsigned int ky = 0, iy = oy; ky < kheight; ky++, iy++) {
                        for (unsigned int kx = 0, ix = ox; kx < kwidth; kx++, ix++) {
                            for (unsigned int inch = 0; inch < inch_sep; inch += 8) {
                                ...
                                __m256 v = _mm256_loadu_ps(kernel_ptr + inch + inchannels * (koutch + kerneloutchannels * (kx + kwidth * ky)));
                            ...
                            if (inch_rem > 0) {
                                ...
                                __m256 v = _mm256_maskload_ps(kernel_ptr + inch_sep + inchannels * (koutch + kerneloutchannels * (kx + kwidth * ky)), mask);
                    ...
                    _mm_maskstore_ps(outmap_ptr + outch + outchannels * (ox + outwidth * oy), mask2, _mm256d_sumwise2(uv_hi, uv_lo));
                }
            }
        }

        inmap_ptr += inchannels * inwidth * inheight;
        outmap_ptr += outchannels * outwidth * outheight;
    }
}

void complex_convolution_2d_grad(unsigned int inchannels, unsigned int outchannels,
                                 unsigned int inwidth, unsigned int outwidth, unsigned int kwidth,
                                 unsigned int inheight, unsigned int outheight, unsigned int kheight,
                                 unsigned int batch, const float* __restrict inmap_ptr, float* __restrict outmap_ptr, const float* __restrict kernel_ptr) {
...
void TensorShaderAvxBackend::Complex::Convolution2D(unsigned int inchannels, unsigned int outchannels, unsigned int inwidth, unsigned int inheight, 
    ...
    if (gradmode) {
        complex_convolution_2d_grad(inchannels, outchannels, 
        ...
        complex_convolution_2d(inchannels, outchannels, 
                               inwidth, outwidth, kwidth,
                               inheight, outheight, kheight,
                               batch, 
                               inmap_ptr, outmap_ptr, kernel_ptr);
