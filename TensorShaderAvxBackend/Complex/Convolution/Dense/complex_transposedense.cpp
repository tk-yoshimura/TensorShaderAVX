...
void complex_transposedense(unsigned int inchannels, unsigned int outchannels, unsigned int th, const float* __restrict inmap_ptr, float* __restrict outmap_ptr, const float* __restrict kernel_ptr) {
    ...
    for (unsigned int outch = 0; outch < outchannels; outch += 2) {
        ...
        for (unsigned int inch = 0; inch < inch_sep; inch += 8) {
            ...
            __m256 v = _mm256_i32gather_ps(kernel_ptr + outch + outchannels * inch / 2, index, 4);
        ...
        if (inch_rem > 0) {
            ...
            __m256 v = _mm256_mask_i32gather_ps(_mm256_setzero_ps(), kernel_ptr + outch + outchannels * inch_sep / 2,
...
void complex_transposedense_grad(unsigned int inchannels, unsigned int outchannels, unsigned int th, const float* __restrict inmap_ptr, float* __restrict outmap_ptr, const float* __restrict kernel_ptr) {
    const unsigned int inmap_offset = inchannels * th, outmap_offset = outchannels * th;
    const unsigned int inch_sep = inchannels & ~7u, inch_rem = inchannels - inch_sep;
    const __m256i mask = TensorShaderAvxBackend::masktable_m256(inch_rem);
    const __m128i mask2 = TensorShaderAvxBackend::masktable_m128(2);
