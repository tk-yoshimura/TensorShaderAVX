...
__forceinline __m256 _mm256_complexmulgrad_ps(__m256 u, __m256 v) {
    __m256 uri = u;
...
void complex_mulgrad(unsigned int length, float* src1_ptr, float* src2_ptr, float* dst_ptr) {
    ...
    for (unsigned int i = 0; i < j; i += 8) {
        ...
        __m256 y = _mm256_complexmulgrad_ps(x1, x2);
...
void TensorShaderAvxBackend::Complex::MulGrad(unsigned int index, unsigned int length, cli::array<float>^ src1, cli::array<float>^ src2, cli::array<float>^ dst) {
    ...
    complex_mulgrad(length, src1_ptr + index, src2_ptr + index, dst_ptr + index);
}