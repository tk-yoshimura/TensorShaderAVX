...
void TensorShaderAvxBackend::Complex::Decay(unsigned int index, unsigned int length, cli::array<float>^ src, cli::array<float>^ dst) {
    ...
    complex_decay(length, src_ptr + index, dst_ptr + index);
}