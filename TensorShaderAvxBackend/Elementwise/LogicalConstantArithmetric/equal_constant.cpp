...
void TensorShaderAvxBackend::Elementwise::EqualConstant(unsigned int index, unsigned int length, float c, cli::array<float>^ src, cli::array<float>^ dst) {
    ...
    equal_constant(length, c, src_ptr + index, dst_ptr + index);
}
