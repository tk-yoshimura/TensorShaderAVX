...
void TensorShaderAvxBackend::Elementwise::GreaterThanOrEqualConstant(unsigned int index, unsigned int length, float c, cli::array<float>^ src, cli::array<float>^ dst) {
    ...
    greaterthanorequal_constant(length, c, src_ptr + index, dst_ptr + index);
}
