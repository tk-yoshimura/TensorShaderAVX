...
void TensorShaderAvxBackend::Elementwise::GreaterThanOrEqual(unsigned int index, unsigned int length, cli::array<float>^ src1, cli::array<float>^ src2, cli::array<float>^ dst) {
    ...
    greaterthanorequal(length, src1_ptr + index, src2_ptr + index, dst_ptr + index);
}
