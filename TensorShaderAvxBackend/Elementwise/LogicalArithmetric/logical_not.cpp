...
void TensorShaderAvxBackend::Elementwise::LogicalNot(unsigned int index, unsigned int length, cli::array<float>^ src, cli::array<float>^ dst) {
    ...
    logical_not(length, src_ptr + index, dst_ptr + index);
}