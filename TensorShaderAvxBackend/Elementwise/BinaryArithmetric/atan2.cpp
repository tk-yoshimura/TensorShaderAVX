...
void atan2(unsigned int length, const float* __restrict src1_ptr, const float* __restrict src2_ptr, float* __restrict dst_ptr) {
    ...
    for (unsigned int i = 0; i < j; i += 8) {
        ...
        __m256 y = _mm256_atan2_ps(x1, x2);
...
void TensorShaderAvxBackend::Elementwise::ArcTan2(unsigned int length, AvxArray<float>^ src1, AvxArray<float>^ src2, AvxArray<float>^ dst) {
    ...
    atan2(length, src1_ptr, src2_ptr, dst_ptr);
}
