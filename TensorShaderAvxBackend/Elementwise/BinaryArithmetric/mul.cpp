...
void mul(unsigned int length, float* src1_ptr, float* src2_ptr, float* dst_ptr) {
...
void TensorShaderAvxBackend::Elementwise::Mul(unsigned int index, unsigned int length, cli::array<float>^ src1, cli::array<float>^ src2, cli::array<float>^ dst) {
    ...
    mul(length, src1_ptr + index, src2_ptr + index, dst_ptr + index);
}