...
void TensorShaderAvxBackend::Elementwise::Pow(unsigned int index, unsigned int length, cli::array<float>^ src1, cli::array<float>^ src2, cli::array<float>^ dst) {
    
    Util::CheckOutOfRange(index, length, src1, src2, dst);
    
    if (length == 1) {
        dst[index] = (float)Math::Pow(src1[index], src2[index]);
        return;
    }
    ...
    pow(length, src1_ptr + index, src2_ptr + index, dst_ptr + index);
}