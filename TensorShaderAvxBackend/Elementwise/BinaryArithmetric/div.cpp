...
void div(unsigned int length, const float* __restrict src1_ptr, const float* __restrict src2_ptr, float* __restrict dst_ptr) {
...
void TensorShaderAvxBackend::Elementwise::Div(unsigned int length, AvxArray<float>^ src1, AvxArray<float>^ src2, AvxArray<float>^ dst) {
    ...
    div(length, src1_ptr, src2_ptr, dst_ptr);
}