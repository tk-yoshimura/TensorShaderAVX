...
void maximum_constant(unsigned int length, float c, float* src_ptr, float* dst_ptr) {
    ...
    for (unsigned int i = 0; i < j; i += 8) {
        ...
        __m256 y = _mm256_max_ps(fillc, x);
...
void TensorShaderAvxBackend::Elementwise::MaximumConstant(unsigned int index, unsigned int length, float c, cli::array<float>^ src, cli::array<float>^ dst) {
    
    Util::CheckOutOfRange(index, length, src, dst);
    
    if (length == 1) {
        dst[index] = Math::Max(c, src[index]);
        return;
    }
    ...
    maximum_constant(length, c, src_ptr + index, dst_ptr + index);
}