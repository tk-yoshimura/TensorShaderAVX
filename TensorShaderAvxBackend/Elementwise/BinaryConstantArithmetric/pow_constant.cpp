...
void TensorShaderAvxBackend::Elementwise::PowConstant(unsigned int index, unsigned int length, float c, cli::array<float>^ src, cli::array<float>^ dst) {
    
    Util::CheckOutOfRange(index, length, src, dst);
    
    if (length == 1) {
        dst[index] = (float)Math::Pow(src[index], c);
        return;
    }
    ...
    pow_constant(length, c, src_ptr + index, dst_ptr + index);
}
