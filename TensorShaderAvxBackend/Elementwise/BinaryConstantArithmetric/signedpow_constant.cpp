...
void TensorShaderAvxBackend::Elementwise::SignedPowConstant(unsigned int index, unsigned int length, float c, cli::array<float>^ src, cli::array<float>^ dst) {
    
    Util::CheckOutOfRange(index, length, src, dst);
    
    if (length == 1) {
        dst[index] = float::IsNaN(src[index]) ? (float)0 : (float)Math::Sign(src[index])
                   * (float)Math::Pow(Math::Abs(src[index]), c);
        return;
    }
    ...
    signedpow_constant(length, c, src_ptr + index, dst_ptr + index);
}
