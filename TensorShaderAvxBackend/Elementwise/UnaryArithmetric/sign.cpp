...
void TensorShaderAvxBackend::Elementwise::Sign(unsigned int index, unsigned int length, cli::array<float>^ src, cli::array<float>^ dst) {
    ...
    sign(length, src_ptr + index, dst_ptr + index);
}