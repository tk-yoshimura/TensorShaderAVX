...
void TensorShaderAvxBackend::Elementwise::NanAsZero(unsigned int index, unsigned int length, cli::array<float>^ src, cli::array<float>^ dst) {
    
    Util::CheckOutOfRange(index, length, src, dst);
    
    if (length == 1) {
        dst[index] = float::IsNaN(src[index]) ? (float)0 : src[index];
        return;
    }
    ...
    nanaszero(length, src_ptr + index, dst_ptr + index);
}