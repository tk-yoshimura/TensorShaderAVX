...
void TensorShaderAvxBackend::Elementwise::Rsqrt(unsigned int index, unsigned int length, cli::array<float>^ src, cli::array<float>^ dst) {
    
    Util::CheckOutOfRange(index, length, src, dst);
    
    if (length == 1) {
        dst[index] = (float)(1 / Math::Sqrt(src[index]));
        return;
    }
    ...
    rsqrt(length, src_ptr + index, dst_ptr + index);
}