...
void TensorShaderAvxBackend::Elementwise::ArcTan(unsigned int index, unsigned int length, cli::array<float>^ src, cli::array<float>^ dst) {
    
    Util::CheckOutOfRange(index, length, src, dst);
    
    if (length == 1) {
        dst[index] = (float)Math::Atan(src[index]);
        return;
    }
    ...
    arctan(length, src_ptr + index, dst_ptr + index);
}