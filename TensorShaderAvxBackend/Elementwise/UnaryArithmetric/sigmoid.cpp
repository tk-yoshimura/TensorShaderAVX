...
void sigmoid(unsigned int length, const float* __restrict src_ptr, float* __restrict dst_ptr) {
    const unsigned int j = length & ~7u, k = length - j;
    
    for (unsigned int i = 0; i < j; i += 8) {
        __m256 x = _mm256_load_ps(src_ptr + i);

        __m256 y = _mm256_sigmoid_ps(x);
...
void TensorShaderAvxBackend::Elementwise::Sigmoid(unsigned int length, AvxArray<float>^ src, AvxArray<float>^ dst) {
