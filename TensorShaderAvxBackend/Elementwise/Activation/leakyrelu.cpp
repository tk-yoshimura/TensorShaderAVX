...
void TensorShaderAvxBackend::Elementwise::LeakyRelu(unsigned int index, unsigned int length, float slope, cli::array<float>^ src, cli::array<float>^ dst) {
    
    Util::CheckOutOfRange(index, length, src, dst);
    
    if (length == 1) {
        dst[index] = (src[index] > 0) ? src[index] : src[index] * slope;
        return;
    }
    ...
    leakyrelu(length, slope, src_ptr + index, dst_ptr + index);
}