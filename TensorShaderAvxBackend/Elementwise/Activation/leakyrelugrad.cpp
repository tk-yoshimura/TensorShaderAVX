...
void leakyrelugrad(unsigned int length, float slope, float* src1_ptr, float* src2_ptr, float* dst_ptr) {
...
void TensorShaderAvxBackend::Elementwise::LeakyReluGrad(unsigned int index, unsigned int length, float slope, cli::array<float>^ src1, cli::array<float>^ src2, cli::array<float>^ dst) {
    
    Util::CheckOutOfRange(index, length, src1, src2, dst);
    
    if (length == 1) {
        dst[index] = (src2[index] > 0) ? src1[index] : src1[index] * slope;
        return;
    }
    ...
    leakyrelugrad(length, slope, src1_ptr + index, src2_ptr + index, dst_ptr + index);
}
