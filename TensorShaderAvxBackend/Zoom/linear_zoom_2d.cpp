...
void TensorShaderAvxBackend::Zoom::LinearZoom2D(unsigned int channels, unsigned int inwidth, unsigned int inheight, unsigned int batch, AvxArray<float>^ inmap, AvxArray<float>^ outmap) {
    ...
    linear_zoom_2d(channels, inwidth, inheight, outwidth, outheight, batch, inmap_ptr, outmap_ptr);
}
