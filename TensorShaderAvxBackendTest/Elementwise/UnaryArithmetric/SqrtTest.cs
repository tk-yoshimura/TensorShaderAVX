    ...
    public class SqrtTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                float[] x = (new float[length + 2]).Select((_) => (float)rd.NextDouble() + 1).ToArray();
                float[] y = new float[length + 2];

                Elementwise.Sqrt(1, (uint)length, x, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(Math.Sqrt(x[i]), y[i], 1e-4f, $"index:{i}");
                }
        ...
        public void DenormalizedTest() {
            ...
            Elementwise.Sqrt(0, (uint)x.Length, x, y);
        ...
        public void SpeedTest() {
            ...
            Elementwise.Sqrt(0, (uint)length, x, y);
