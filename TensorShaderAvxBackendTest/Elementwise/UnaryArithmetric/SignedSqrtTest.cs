    ...
    public class SignedSqrtTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                ...
                Elementwise.SignedSqrt(1, (uint)length, x, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(Math.Sign(x[i]) * Math.Sqrt(Math.Abs(x[i])), y[i], 1e-4f, $"index:{i}");
                }
        ...
        public void DenormalizedTest() {
            ...
            Elementwise.SignedSqrt(0, (uint)x.Length, x, y);
        ...
        public void SpeedTest() {
            ...
            Elementwise.SignedSqrt(0, (uint)length, x, y);
