    ...
    public class CbrtTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                ...
                Elementwise.Cbrt(1, (uint)length, x, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(Math.Sign(x[i]) * Math.Pow(Math.Abs(x[i]), 1.0 / 3.0), y[i], 1e-4f, $"index:{i}");
                }
        ...
        public void DenormalizedTest() {
            ...
            Elementwise.Cbrt(0, (uint)x.Length, x, y);
        ...
        public void SpeedTest() {
            ...
            Elementwise.Cbrt(0, (uint)length, x, y);
