    ...
    public class Log10Test {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                float[] x = (new float[length + 2]).Select((_) => (float)rd.NextDouble() * 10 + 1).ToArray();
                float[] y = new float[length + 2];

                Elementwise.Log10(1, (uint)length, x, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(Math.Log(x[i], 10), y[i], 1e-3f, $"index:{i}");
                }
        ...
        public void DenormalizedTest() {
            ...
            Elementwise.Log10(0, (uint)x.Length, x, y);
        ...
        public void SpeedTest() {
            ...
            Elementwise.Log10(0, (uint)length, x, y);
