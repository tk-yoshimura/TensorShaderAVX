    ...
    public class LogCoshTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                ...
                Elementwise.LogCosh(1, (uint)length, x, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(Math.Log(Math.Cosh(x[i])), y[i], 1e-3f, $"index:{i}");
                }
        ...
        public void DenormalizedTest() {
            ...
            Elementwise.LogCosh(0, (uint)x.Length, x, y);

            Assert.AreEqual(0, y[0]);
            Assert.AreEqual(Math.Log(Math.Cosh(1)), y[1], 1e-4f);
            Assert.AreEqual(Math.Log(Math.Cosh(-1)), y[2], 1e-4f);
        ...
        public void SpeedTest() {
            ...
            Elementwise.LogCosh(0, (uint)length, x, y);
