    ...
    public class SigmoidTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                ...
                Elementwise.Sigmoid(1, (uint)length, x, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(1 / (1 + Math.Exp(-x[i])), y[i], 1e-3f, $"index:{i}");
                }
        ...
        public void DenormalizedTest() {
            ...
            Elementwise.Sigmoid(0, (uint)x.Length, x, y);

            Assert.AreEqual(0.5f, y[0]);
            Assert.AreEqual(1 / (1 + Math.Exp(-1)), y[1], 1e-4f);
            Assert.AreEqual(1 / (1 + Math.Exp(1)), y[2], 1e-4f);
        ...
        public void SpeedTest() {
            ...
            Elementwise.Sigmoid(0, (uint)length, x, y);
