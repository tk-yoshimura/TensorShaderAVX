    ...
    public class ArcCosTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                float[] x = (new float[length + 2]).Select((_) => (float)rd.NextDouble() * 1.5f - 0.75f).ToArray();
                float[] y = new float[length + 2];

                Elementwise.ArcCos(1, (uint)length, x, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(Math.Acos(x[i]), y[i], 1e-3f, $"index:{i}");
                }
        ...
        public void DenormalizedTest() {
            ...
            Elementwise.ArcCos(0, (uint)x.Length, x, y);

            Assert.AreEqual(Math.PI / 2, y[0], 1e-4f);
            Assert.AreEqual(0, y[1]);
            Assert.AreEqual(Math.PI, y[2], 1e-4f);
        ...
        public void SpeedTest() {
            ...
            Elementwise.ArcCos(0, (uint)length, x, y);
