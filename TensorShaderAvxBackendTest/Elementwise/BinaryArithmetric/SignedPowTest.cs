    ...
    public class SignedPowTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                float[] x1 = (new float[length + 2]).Select((_) => (float)rd.NextDouble() * 10 - 5).ToArray();
                float[] x2 = (new float[length + 2]).Select((_) => (float)rd.NextDouble() + 1f).ToArray();
                float[] y = new float[length + 2];

                Elementwise.SignedPow(1, (uint)length, x1, x2, y);
        ...
        public void SpeedTest() {
            {
                ...
                Elementwise.SignedPow(0, (uint)length, x1, x2, y);
