    ...
    public class SignedPowTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                float[] x1 = (new float[length + 1]).Select((_) => (float)rd.NextDouble() * 10 - 5).ToArray();
                float[] x2 = (new float[length + 1]).Select((_) => (float)rd.NextDouble() + 1f).ToArray();
                float[] y = new float[length + 1];

                AvxArray<float> vx1 = x1, vx2 = x2, vy = y;

                Elementwise.SignedPow((uint)length, vx1, vx2, vy);

                y = vy;

                for(int i = 0; i < length; i++) {
                    Assert.AreEqual(Math.Sign(x1[i]) * Math.Pow(Math.Abs(x1[i]), x2[i]), y[i], 1e-4f, $"index:{i}");
                }
