    ...
    public class LogicalXorTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                ...
                float[] x2 = (new float[length + 2]).Select((_) => (float)rd.NextDouble()).ToArray();
                float[] y = new float[length + 2];

                Elementwise.LogicalXor(1, (uint)length, x1, x2, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(Math.Max(x1[i], x2[i]) - Math.Min(x1[i], x2[i]), y[i], $"index:{i}");
                }
        ...
        public void SpeedTest() {
            ...
            Elementwise.LogicalXor(0, (uint)length, x1, x2, y);
