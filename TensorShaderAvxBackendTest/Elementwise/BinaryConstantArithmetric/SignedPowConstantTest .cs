...
namespace TensorShaderAvxBackendTest.ElementwiseBinaryConstantArithmetric {
    [TestClass]
    public class SignedSignedPowConstantTest {
        [TestMethod]
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                ...
                Elementwise.SignedPowConstant(1, (uint)length, 2, x, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(Math.Sign(x[i]) * x[i] * x[i], y[i], 1e-4f, $"index:{i}");
                }
        ...
        public void SpeedTest() {
            ...
            Elementwise.SignedPowConstant(0, (uint)length, 2, x, y);
