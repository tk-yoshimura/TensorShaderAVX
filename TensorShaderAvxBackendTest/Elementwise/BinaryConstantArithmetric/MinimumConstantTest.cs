...
namespace TensorShaderAvxBackendTest.ElementwiseBinaryConstantArithmetric {
    [TestClass]
    public class MinimumConstantTest {
        [TestMethod]
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                ...
                Elementwise.MinimumConstant(1, (uint)length, 0.5f, x, y);

                for(int i = 1; i <= length; i++) {
                    Assert.AreEqual(Math.Min(0.5f, x[i]), y[i], $"index:{i}");
                }
        ...
        public void SpeedTest() {
            ...
            Elementwise.MinimumConstant(0, (uint)length, 0.5f, x, y);
