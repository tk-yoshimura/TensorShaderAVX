    ...
    public class DivConstantTest {
        ...
        public void ExecuteTest() {
            ...
            for(int length = 0; length < 1000; length++) {
                float[] x = (new float[length + 2]).Select((_) => (float)rd.NextDouble() + 1f).ToArray();
                float[] y = new float[length + 2];

                Elementwise.DivConstant(1, (uint)length, 2, x, y);
        ...
        public void SpeedTest() {
            {
                ...
                Elementwise.DivConstant(0, (uint)length, 2, x, y);
